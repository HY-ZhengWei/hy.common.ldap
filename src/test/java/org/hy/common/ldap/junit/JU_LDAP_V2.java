 ...
 * 测试：LDAP目录服务的操作类
 * 
 * 当时的测试环境为：OpenLDAP 2.4.46
 * 使用API为：Apache LDAP API 2.0.0.AM2 未使用OpenLDAP的 JLDAP 库。
 * 
...
public class JU_LDAP_V2
{
    ...
    public JU_LDAP_V2() throws Exception
    {
        if ( !$isInit )
        {
            $isInit = true;
            XJava.parserAnnotation(JU_LDAP_V2.class.getName());
        }
    }
